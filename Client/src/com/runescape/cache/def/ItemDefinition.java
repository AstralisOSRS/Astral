...
public final class ItemDefinition {
	...
	public void readValues(Buffer buffer) {
		do {
			...
			String info = "OpCode: " + opCode + "\n";
			if (opCode == 0)
				return;
			if (opCode == 1) {
				inventory_model = buffer.readUShort();
				info += inventory_model;
			} else if (opCode == 2) {
				...
				equipped_model_male_translation_y = (byte) buffer.readUnsignedByte();
				info += equipped_model_male_1 + " | " + equipped_model_male_translation_y;
			} else if (opCode == 24) {
				equipped_model_male_2 = buffer.readUShort();
				info += equipped_model_male_2;
			} else if (opCode == 25) {
				equipped_model_female_1 = buffer.readUShort();
				equipped_model_female_translation_y = (byte) buffer.readUnsignedByte();
				info += equipped_model_female_1 + " | " + equipped_model_female_translation_y;
			} else if (opCode == 26)
				equipped_model_female_2 = buffer.readUShort();
			else if (opCode >= 30 && opCode < 35) {
				if (groundActions == null)
					groundActions = new String[5];
				groundActions[opCode - 30] = buffer.readNewString();
				if (groundActions[opCode - 30].equalsIgnoreCase("hidden"))
					groundActions[opCode - 30] = null;
				info += groundActions[opCode - 30];
			} else if (opCode >= 35 && opCode < 40) {
				if (actions == null)
					actions = new String[5];
				actions[opCode - 35] = buffer.readString();
				info += actions[opCode - 35];
			} else if (opCode == 40) {
				...
				for (int k = 0; k < j; k++) {
					...
					info += original_model_colors[k] + " | " + modified_model_colors[k];
				}
			} else if(opCode == 42) {
				System.out.println("Op-42: " + (buffer.readSignedByte()));//we don't know what this is
			} else if(opCode == 65) {
				boolean something = true;//we dont know what this is
			} else if (opCode == 78)
				...
				light_mag = buffer.readSignedByte() * 5;//idk if *5 still required
			else if (opCode == 115)
				team = buffer.readUnsignedByte();
			else if (opCode == 139)
				System.out.println("Op-139: " + (buffer.readUShort()));//this.field3696 = paramclass194.method3539();
	        else if (opCode == 140)
	        		System.out.println("Op-140: " + (buffer.readUShort()));//this.field3697 = paramclass194.method3539();
	        else if (opCode == 148)
	        		System.out.println("Op-148: " + (buffer.readUShort()));//this.field3698 = paramclass194.method3539();
	        else if (opCode == 149)
	        		System.out.println("Op-149: " + (buffer.readUShort()));//this.field3699 = paramclass194.method3539();
	        else if (opCode == 249) {
	              buffer.readUnsignedByte();//this.field3694 = class2.method5(paramclass194, this.field3694);
	        }
